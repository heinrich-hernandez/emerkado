    ...
    <div class="d-flex align-items-center">
      ...
      <div class="ml-auto">
        <a href="javascript:void(0)"
          class="btn {{ $user->review_status == 'For Review' ? 'btn-danger' : ($user->review_status == 'In Progress' ? 'btn-warning' : 'btn-success') }} approval-button fw-bolder " 
          data-toggle="modal"
          data-target="#modal-default">
            <!-- Approval : {{ $user->review_status == 'For Review' ? 'Unassigned' : $user->review_status }} -->
            Approval : {{ $user->review_status }}
        </a>
      </div>
    </div>  
    @if ($errors->any())
...
<!-- Modal -->
<form action="{{ route('approved.review_coop', $user->id ) }}" method="post" enctype="multipart/form-data">
@csrf
          ...
          <div class="col-md-3">
              ...
              <div class="card-body box-profile">
                <div class="text-center">
                  <img class="profile-user-img img-fluid img-circle"
                       src="{{ $user->profile_picture ? URL::to('/storage') . '/' . $user->profile_picture : asset('images/guest.jpg') }}" alt="User profile picture">
                </div>

                <h3 class="profile-username text-center">{{ $user->authorized_representative }}</h3>

                <p class="text-muted text-center">{{ $user->name }}</p>

                <ul class="list-group list-group-unbordered mb-3">
                  <li class="list-group-item">
                    <b>Account Created</b> <a class="float-right">{{ $user->created_at->format('d/m/Y') }}</a>
                  </li>
                  <li class="list-group-item">
                    <b>Account Activation</b> <a class="float-right">{{ $user->status ? 'Activated' : 'For Activation' }}</a>
                  </li>
                  <li class="list-group-item">
                    <b>Account Status</b> <a class="float-right">{{ $user->review_status == 'For Review' ? 'Unassigned' : $user->review_status }}</a>
                  </li>
                </ul>
            ...
            <div class="card card-primary">
              ...
              <div class="card-body">
                ...
                <p class="text-muted">{{ $user->business_description }}</p>
              </div>
              <!-- /.card-body -->
            </div>
            <!-- /.card -->
                      ...
                      <div class="active tab-pane" id="activity" >
                        ...
                        <tr>
                          ...
                          <tr>
                            ...
                            <td>:</td>
                            <td>{{ $user->authorized_representative }}</td>
                          </tr>
                          <tr>
                          ...
                          @if($user->agency_affiliation =='yes')
                          <tr>
                            <td style="font-weight: 800;">Agency Affiliation Name</td>
                            <td>:</td>
                            <td>{{ $user->agency_affiliation_name }}</td>
                          </tr>
                          @endif
                          <tr>
